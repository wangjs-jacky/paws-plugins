...
function providerMessages(messages: RelationshipAdvisorMessage[], imageUrls: string[]) {
    let lastUserIndex = -1;
...
export async function* streamRelationshipAdvisor(
    ...
    if (!response.body) throw new Error('Relationship advisor provider returned no stream');
